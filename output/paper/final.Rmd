...
Since America current has 330 million population, I will run the Simulation 330 times.
```{r, echo = FALSE, message = FALSE, warning = FALSE}
set.seed(853)
simulated_data <-
  tibble( 
...
56
    ...
    BMI = 
      runif(n = 330,
            min = 18.5, 
            max = 186),
    sleeping =  sample(x=0:24, 330,replace = TRUE),
    health =  sample(x=1:5, 330,replace = TRUE),
    sex =  sample(x=0:1, 330,replace = TRUE),
    smoke =  sample(x=0:1, 330,replace = TRUE),
    age =  sample(x=18:100, 330,replace = TRUE),
    exercise =  sample(x=0:1, 330,replace = TRUE),
    alcohol =  sample(x=0:1, 330,replace = TRUE)
          )
head(simulated_data) %>% 
  kable(caption = "summary statistic of Body Mass Index") %>%
  kable_classic(full_width = F, html_font = "Cambria") %>%
